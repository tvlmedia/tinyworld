...
export function updateHousingUpgrades(state: GameState, dt: number): void {
  ...
  const candidate = findUpgradeCandidate(state);
  if (!candidate) return;
...
function findUpgradeCandidate(state: GameState): { house: Building; upgrade: HouseUpgrade } | undefined {
  const houses = state.buildings
    ...
    .sort((a, b) => (a.upgradeLevel ?? 1) - (b.upgradeLevel ?? 1));
